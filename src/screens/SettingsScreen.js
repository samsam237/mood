...
const SettingsScreen = ({ navigation }) => {
  console.log('⚙️ SettingsScreen MOBILE version loaded!');
  const { user, signOut } = useAuth();
  const { t } = useTranslation();
  const { alert, showError, showSuccess, hideAlert } = useCustomAlert();
  const { confirmation, showDangerConfirmation, hideConfirmation, handleConfirm } = useCustomConfirmation();
  ...
  const handleSignOut = async () => {
    showDangerConfirmation(
      ...
      async () => {
        ...
        if (!result.success) {
          showError(t('settings.signOutError'), t('settings.signOutErrorMessage'));
        }
      }
    );
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.signOutContainer}>
          <TouchableOpacity style={styles.signOutButton} onPress={handleSignOut}>
            ...
            <Text style={styles.signOutText}>{t('settings.disconnect')}</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.versionContainer}>
          ...
          <Text style={styles.versionSubtext}>{t('settings.appSubtitle')}</Text>
        </View>
